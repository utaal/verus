...
pub(crate) fn export_crate(args: &Args, vir_metadata: CrateMetadata, vir_crate: Krate) -> Result<(), VirErr> {
    if let Some(file_path) = &args.export {
        ...
        let krate_with_metadata = CrateWithMetadata {
            krate: vir_crate,
            metadata: vir_metadata,
        };
