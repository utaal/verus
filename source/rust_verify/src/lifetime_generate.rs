...
use crate::verus_items::{PervasiveItem, RustItem, VerusItem};
...
fn erase_ty<'tcx>(ctxt: &Context<'tcx>, state: &mut State, ty: &Ty<'tcx>) -> Typ {
    match ty.kind() {
        ...
        TyKind::Adt(AdtDef(adt_def_data), args) => {
            ...
            let path = def_id_to_vir_path(ctxt.tcx, &ctxt.type_ctxt, did);
        ...
        TyKind::Alias(rustc_middle::ty::AliasKind::Projection, t) => {
            ...
            match (trait_def, t.substs.try_as_type_list()) {
                (Some(trait_def), Some(typs)) if typs.len() >= 1 => {
                    let trait_path_vir = def_id_to_vir_path(ctxt.tcx, &ctxt.type_ctxt, trait_def);
...
fn erase_pat<'tcx>(ctxt: &Context<'tcx>, state: &mut State, pat: &Pat) -> Pattern {
    ...
    match &pat.kind {
        ...
        PatKind::Path(qpath) => {
            ...
            let vir_path = def_id_to_vir_path(ctxt.tcx, &ctxt.type_ctxt, adt_def_id);
...
fn erase_inv_block<'tcx>(
    ...
    let (_guard_hir, _inner_hir, inner_pat, arg, atomicity) =
        crate::rust_to_vir_expr::invariant_block_open(&ctxt.type_ctxt, open_stmt)
            ...
            match res {
                ...
                Res::Def(DefKind::Const, id) => {
                    if expect_spec || ctxt.var_modes[&expr.hir_id] == Mode::Spec {
                        ...
                        let vir_path = def_id_to_vir_path(ctxt.tcx, &ctxt.type_ctxt, id);
...
fn erase_const<'tcx>(
    ...
    let path = def_id_to_vir_path(ctxt.tcx, &ctxt.type_ctxt, id);
    ...
    for (pred, _) in mir_predicates.predicates.iter() {
        match pred.kind().no_bound_vars().expect("no_bound_vars") {
            ...
            PredicateKind::Clause(Clause::Trait(pred)) => {
                ...
                let trait_path = def_id_to_vir_path(ctxt.tcx, &ctxt.type_ctxt, id);
...
fn erase_trait<'tcx>(
    ...
    let path = def_id_to_vir_path(ctxt.tcx, &ctxt.type_ctxt, trait_id);
        ...
        match impl_item_ref.kind {
            ...
            AssocItemKind::Type => {
                    ...
                    (&impl_item.kind, &impll.of_trait)
                {
                    ...
                    let trait_path_vir =
                        def_id_to_vir_path(ctxt.tcx, &ctxt.type_ctxt, path.res.def_id());
...
fn erase_mir_datatype<'tcx>(ctxt: &Context<'tcx>, state: &mut State, id: DefId) {
    ...
    let verus_item = ctxt.type_ctxt.verus_id_to_name(&id);
